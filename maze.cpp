...
class cell{
    ...
    bool operater== (const Cell& c) const {
        return x == c.x && y == c.y;
    }
    private:
    int x, y;
...
class Maze{
    ...
    void pushUnvisted(int,int);
    friend ostream& operator<< (ostream&, const Maze&);
    int rows, cols;
};

Maze::Maze() : exitMarker('e'), entryMarker('m'), visted('.'), 
passage('0'), wall('1') {
    Stack<char*> mazeRows;
    int col, row = 0;
    cout << "Enter a rectangular maze using the following "
         ...
         rows = row;
         store = new char*[rows+2];      // create 1D array of pointers
...
void Maze::pushUnvisted(int row, int col) {
    if (store[col] == passage || store[row][col] == exitMarker) {
